...
public class IntegrationActivityLog : ActivityLogFixtureBase
{
    ...
    public async Task AddNode_RootTaskNode_Persisted()
    {
        await Run<IActivityLogDataProvider>(async provider =>
        {
            ...
            tree[0].NodeType.ShouldBe(DeploymentActivityLogNodeType.Task);
    ...
    public async Task BuildTree_TaskStepActionHierarchy_Persisted()
    {
        await Run<IActivityLogDataProvider>(async provider =>
        {
            ...
            var step1 = await provider.AddNodeAsync(new ActivityLogEntity
            {
                ...
                NodeType = DeploymentActivityLogNodeType.Step,
                Status = DeploymentActivityLogNodeStatus.Running,
                StartedAt = DateTimeOffset.UtcNow,
                SortOrder = 1
            }).ConfigureAwait(false);
            ...
            var step2 = await provider.AddNodeAsync(new ActivityLogEntity
            {
                ...
                NodeType = DeploymentActivityLogNodeType.Step,
                Status = DeploymentActivityLogNodeStatus.Pending,
                StartedAt = DateTimeOffset.UtcNow,
                SortOrder = 2
            }).ConfigureAwait(false);
            ...
            var action1 = await provider.AddNodeAsync(new ActivityLogEntity
            {
                ...
                NodeType = DeploymentActivityLogNodeType.Action,
                Status = DeploymentActivityLogNodeStatus.Running,
                StartedAt = DateTimeOffset.UtcNow,
                SortOrder = 1
            }).ConfigureAwait(false);
            ...
            await provider.AddNodeAsync(new ActivityLogEntity
            {
                ...
                ParentId = action1.Id,
                Name = "Script output",
                NodeType = DeploymentActivityLogNodeType.LogEntry,
                Category = DeploymentActivityLogCategory.Info,
            ...
            step1Children[0].NodeType.ShouldBe(DeploymentActivityLogNodeType.Action);
            ...
            action1Children[0].NodeType.ShouldBe(DeploymentActivityLogNodeType.LogEntry);
    ...
    public async Task UpdateNodeStatus_RunningToSuccess_Updated()
    {
        await Run<IActivityLogDataProvider>(async provider =>
        {
            ...
            node.Status.ShouldBe(DeploymentActivityLogNodeStatus.Running);
            node.EndedAt.ShouldBeNull();

            var endTime = DateTimeOffset.UtcNow;
            await provider.UpdateNodeStatusAsync(node.Id, DeploymentActivityLogNodeStatus.Success, endTime).ConfigureAwait(false);

            var updated = await provider.GetNodeByIdAsync(node.Id).ConfigureAwait(false);
            updated.Status.ShouldBe(DeploymentActivityLogNodeStatus.Success);
    ...
    public async Task UpdateNodeStatus_RunningToFailed_Updated()
    {
        await Run<IActivityLogDataProvider>(async provider =>
        {
            ...
            await provider.UpdateNodeStatusAsync(node.Id, DeploymentActivityLogNodeStatus.Failed, DateTimeOffset.UtcNow).ConfigureAwait(false);

            var updated = await provider.GetNodeByIdAsync(node.Id).ConfigureAwait(false);
            updated.Status.ShouldBe(DeploymentActivityLogNodeStatus.Failed);
        }).ConfigureAwait(false);
    }
    ...
    public async Task UpdateNodeStatus_NonExistentNode_NoException()
    {
        await Run<IActivityLogDataProvider>(async provider =>
        {
            await provider.UpdateNodeStatusAsync(999999, DeploymentActivityLogNodeStatus.Success, DateTimeOffset.UtcNow).ConfigureAwait(false);
        }).ConfigureAwait(false);
    }
    ...
    public async Task AddNode_WithLogText_Persisted()
    {
        await Run<IActivityLogDataProvider>(async provider =>
        {
            ...
            var node = await provider.AddNodeAsync(new ActivityLogEntity
            {
                ...
                Name = "Script Output",
                NodeType = DeploymentActivityLogNodeType.LogEntry,
                Category = DeploymentActivityLogCategory.Error,
            ...
            retrieved.Category.ShouldBe(DeploymentActivityLogCategory.Error);
            retrieved.NodeType.ShouldBe(DeploymentActivityLogNodeType.LogEntry);
        }).ConfigureAwait(false);
    }
    ...
    public async Task FullDeploymentTreeLifecycle_StatusTransitions()
    {
        await Run<IActivityLogDataProvider>(async provider =>
        {
            ...
            await provider.UpdateNodeStatusAsync(action1.Id, DeploymentActivityLogNodeStatus.Success, now.AddSeconds(5)).ConfigureAwait(false);

            // Mark step as success
            await provider.UpdateNodeStatusAsync(step1.Id, DeploymentActivityLogNodeStatus.Success, now.AddSeconds(6)).ConfigureAwait(false);
            ...
            await provider.UpdateNodeStatusAsync(step2.Id, DeploymentActivityLogNodeStatus.Failed, now.AddSeconds(12)).ConfigureAwait(false);

            // Mark task as failed
            await provider.UpdateNodeStatusAsync(taskNode.Id, DeploymentActivityLogNodeStatus.Failed, now.AddSeconds(12)).ConfigureAwait(false);

            // Verify final state
            var tree = await provider.GetTreeByTaskIdAsync(taskId).ConfigureAwait(false);
            tree.Count.ShouldBe(4);

            var rootNode = tree.First(n => n.NodeType == DeploymentActivityLogNodeType.Task);
            rootNode.Status.ShouldBe(DeploymentActivityLogNodeStatus.Failed);
            rootNode.EndedAt.ShouldNotBeNull();

            var successStep = tree.First(n => n.Name == "Database Migration");
            successStep.Status.ShouldBe(DeploymentActivityLogNodeStatus.Success);

            var failedStep = tree.First(n => n.Name == "Deploy Application");
            failedStep.Status.ShouldBe(DeploymentActivityLogNodeStatus.Failed);

            var successAction = tree.First(n => n.NodeType == DeploymentActivityLogNodeType.Action);
            successAction.Status.ShouldBe(DeploymentActivityLogNodeStatus.Success);
        }).ConfigureAwait(false);
    }
}
