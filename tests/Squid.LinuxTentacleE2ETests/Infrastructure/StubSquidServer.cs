...
// CROSS-PLATFORM SYNC NOTE: Windows project has a near-identical sibling
// at tests/Squid.WindowsTentacleE2ETests/Infrastructure/StubSquidServer.cs.
// Both implement the same Halibut-listener + REST-register surface; the
// only differences are namespace + Linux's slim LinuxStubSquidServer
// companion (REST-only) for register-only tests. Bug fixes here MUST be
// mirrored to the Windows side. Future refactor candidate: extract to a
// shared infra project — kept as parallel copies until either side
// diverges enough to justify the cross-project coupling cost.
