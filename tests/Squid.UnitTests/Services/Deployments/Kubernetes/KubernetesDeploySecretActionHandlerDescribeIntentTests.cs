...
/// <summary>
/// Phase 9c.4 — verifies that <see cref="KubernetesDeploySecretActionHandler"/> overrides
/// <c>DescribeIntentAsync</c> and emits a <see cref="KubernetesApplyIntent"/> directly, with
/// a stable semantic name (<c>k8s-apply</c>) and the generated Secret YAML carried as a
/// single <c>secret.yaml</c> asset. Invalid/unconfigured actions produce an intent with an
/// empty <c>YamlFiles</c> collection instead of a null result. Legacy <c>PrepareAsync</c>
/// is preserved until Phase 9g.
/// </summary>
public class KubernetesDeploySecretActionHandlerDescribeIntentTests
{
    private readonly KubernetesDeploySecretActionHandler _handler = new();

    private static DeploymentActionDto CreateAction(
        string actionName = "deploy-secret",
        string secretName = "my-secret",
        string secretValues = """[{"Key":"DB_PASS","Value":"s3cret"}]""",
        string namespaceValue = null)
    {
        ...
        if (secretName != null)
            ...
            {
                PropertyName = "Squid.Action.KubernetesContainers.SecretName",
                PropertyValue = secretName
            });

        if (secretValues != null)
            action.Properties.Add(new DeploymentActionPropertyDto
            {
                PropertyName = "Squid.Action.KubernetesContainers.SecretValues",
                PropertyValue = secretValues
            });
    ...
    [Fact]
    public async Task DescribeIntentAsync_ValidSecret_YieldsSingleSecretYamlFile()
    {
        var ctx = CreateContext();
        ...
        intent.YamlFiles[0].RelativePath.ShouldBe("secret.yaml");
    }

    [Fact]
    public async Task DescribeIntentAsync_YamlContent_HasKindSecret()
    {
        var ctx = CreateContext();
    ...
    [Fact]
    public async Task DescribeIntentAsync_MissingSecretName_ProducesEmptyYamlFiles()
    {
        var ctx = CreateContext(action: CreateAction(secretName: null));
    ...
    [Fact]
    public async Task DescribeIntentAsync_EmptySecretValues_ProducesEmptyYamlFiles()
    {
        var ctx = CreateContext(action: CreateAction(secretValues: "[]"));
    ...
    {
        var ctx = CreateContext(stepName: "Apply Secret Prod", action: CreateAction(actionName: "prod-secret"));

        var intent = await ((IActionHandler)_handler).DescribeIntentAsync(ctx, CancellationToken.None);

        intent.StepName.ShouldBe("Apply Secret Prod");
        intent.ActionName.ShouldBe("prod-secret");
    }
