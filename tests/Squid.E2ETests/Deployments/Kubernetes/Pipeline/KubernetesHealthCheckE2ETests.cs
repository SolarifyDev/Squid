...
[Trait("Category", "E2E")]
public class KubernetesHealthCheckE2ETests
    : IClassFixture<DeploymentPipelineFixture<KubernetesHealthCheckE2ETests>>
{
    private readonly KindClusterFixture _cluster;
    private readonly DeploymentPipelineFixture<KubernetesHealthCheckE2ETests> _fixture;

    public KubernetesHealthCheckE2ETests(
        KindClusterFixture cluster,
        DeploymentPipelineFixture<KubernetesHealthCheckE2ETests> fixture)
    {
        _cluster = cluster;
        _fixture = fixture;
    }
    ...
    [Fact]
    public async Task Pipeline_MultiTarget_HealthyTargetExecutesDeploy()
    {
        ...
        // The healthy target should have executed the deploy step
        ExecutionCapture.CapturedRequests.ShouldNotBeEmpty("Healthy target should execute the deploy step");
    ...
    private async Task<int> SeedMultiTargetWithHealthCheckAsync()
    {
        ...
        await _fixture.Run<IRepository, IUnitOfWork>(async (repository, unitOfWork) =>
        {
            ...
            await builder.CreateActionPropertiesAsync(action.Id,
                ("Squid.Action.Script.ScriptBody", "echo 'deploying'"),
