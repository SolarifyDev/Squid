...
public class MachineDataProvider(IUnitOfWork unitOfWork, IRepository repository) : IMachineDataProvider
{
    ...
    {
        var query = repository.Query<Persistence.Entities.Deployments.Machine>();
    ...
    {
        return await repository.Query<Persistence.Entities.Deployments.Machine>(x => id == x.Id).FirstOrDefaultAsync(cancellationToken).ConfigureAwait(false);
    }
    ...
    {
        return await repository.Query<Persistence.Entities.Deployments.Machine>(x => ids.Contains(x.Id)).ToListAsync(cancellationToken).ConfigureAwait(false);
    }
    ...
    {
        var query = repository.QueryNoTracking<Persistence.Entities.Deployments.Machine>(m => !m.IsDisabled);
