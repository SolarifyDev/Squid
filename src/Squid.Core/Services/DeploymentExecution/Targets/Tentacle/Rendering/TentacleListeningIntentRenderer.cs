...
public sealed class TentacleListeningIntentRenderer : IIntentRenderer
{
    ...
    public Task<ScriptExecutionRequest> RenderAsync(ExecutionIntent intent, IntentRenderContext context, CancellationToken ct)
    {
        ...
        return intent switch
        {
            ...
            _ => throw new IntentRenderingException(CommunicationStyle, intent, $"TentacleListeningIntentRenderer only supports RunScriptIntent and DeployPackageIntent, got '{intent.GetType().Name}'.")
        };
    }
    ...
    private static ScriptExecutionRequest RenderDeployPackage(DeployPackageIntent intent, IntentRenderContext context)
    {
        var acquired = context.PackageReferences
            ...
            ?? throw new IntentRenderingException(CommunicationStyle.TentacleListening, intent,
