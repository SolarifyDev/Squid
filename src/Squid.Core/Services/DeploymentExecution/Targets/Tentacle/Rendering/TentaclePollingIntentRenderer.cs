...
{
    public CommunicationStyle CommunicationStyle => CommunicationStyle.LinuxPolling;
    ...
    public Task<ScriptExecutionRequest> RenderAsync(ExecutionIntent intent, IntentRenderContext context, CancellationToken ct)
    {
        ...
        if (intent is not RunScriptIntent runScript)
            throw new IntentRenderingException(CommunicationStyle, intent, $"TentaclePollingIntentRenderer only supports RunScriptIntent, got '{intent.GetType().Name}'.");
