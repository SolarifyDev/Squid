...
public class SshHealthCheckStrategy : IHealthCheckStrategy
{
    ...
    public async Task<HealthCheckResult> CheckHealthAsync(Machine machine, MachineConnectivityPolicyDto connectivityPolicy, CancellationToken ct)
    {
        ...
        try
        {
            ...
            return await Task.Run(() => ProbeConnectivity(connectionInfo), ct).ConfigureAwait(false);
        }
        catch (Exception ex)
        {
    ...
    private async Task<SshConnectionInfo> BuildConnectionInfoAsync(Machine machine, SshEndpointDto endpoint, MachineConnectivityPolicyDto connectivityPolicy, CancellationToken ct)
    {
        ...
        return new SshConnectionInfo(endpoint.Host, endpoint.Port, username, privateKey, passphrase, password, endpoint.Fingerprint, timeout);
    }

    private HealthCheckResult ProbeConnectivity(SshConnectionInfo connectionInfo)
    {
